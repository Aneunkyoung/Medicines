...
#updateContent{
		...
		<section class="module">
			<div class="container" >
				<div class="row">
					...
					<div><button onclick="location.href='${pageContext.request.contextPath}/board'" type="button" class="btn btn-border-d btn-round" style="margin-bottom: 10px; float: right;">목록</button></div>
							...
							<tbody>
								...
								<tr>
									<td colspan="2" class="contnet">
										<ul class="etcArea">
						                	<li>
						                    	<strong>조회수</strong> <span>${board.readcount }</span>
						                    </li>
						                </ul>
						                <div class="detail">
						                	<div>
						                		<c:if test="${board.img != null}">
						                		<img src="${pageContext.request.contextPath}/boardImg/${board.img}" style="width: 200px; margin-bottom:10px;">	
						                		</c:if>
						                	</div>
						                	<div>
						                		<p style="white-space: pre;">${board.content }</p>
						                	</div>
						                </div>
									</td>
								</tr>	
						...
						<c:if test="${loginUser.id == board.id  || loginUser.userStatus==9}">		
							<div style="margin-top: 10px; float: right;">			
								<button onclick="removeBoard(${board.num})" type="button" class="btn btn-border-d btn-round">삭제</button>
								<button id="commentModifyBtn" onclick="updateBoard(${board.num})" type="button" class="btn btn-border-d btn-round">변경</button>
							</div>
						</c:if>
					</div>
					...
					<div class="col-sm-12 col-sm-offset-0" style="margin-top: 20px;">
						...
						<form id="commentForm" action="comment_write/${board.num }" method="post">
				...
				<div id="paging" class="pagination font-inc text-uppercase">
							<c:choose>
								<c:when test="${pager.startPage > pager.blockSize}">
									<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=1"><i class='fa fa-angle-left'></i> First </a> 
									<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=${pager.prevPage}"><i class='fa fa-angle-left'></i> Prev </a>
								</c:when>
								
								<c:otherwise>
									<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=1"><i class='fa fa-angle-left'></i> First </a> <i class='fa fa-angle-left'></i> Prev 
								</c:otherwise>
							</c:choose>
							...
							<c:forEach begin="${pager.startPage }" end="${pager.endPage }" var="page">
								<c:choose>
									<c:when test="${page != pager.pageNum }">
										<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=${page }">[${page}]</a>
									</c:when>
									<c:otherwise>
										[${page}]
									</c:otherwise>
							...
							<c:choose>
								<c:when test="${pager.endPage != pager.totalPage}">
									<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=${pager.nextPage}"> Next <i class='fa fa-angle-right'></i></a>
									<a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=${pager.totalPage}"> Last <i class='fa fa-angle-right'></i></a>
								</c:when>
								
								<c:otherwise>
									Next <i class='fa fa-angle-right'></i> <a href="${pageContext.request.contextPath}/board_view?num=${board.num }&pageNum=${pager.totalPage}"> Last <i class='fa fa-angle-right'></i></a>	
								</c:otherwise>
							</c:choose>
						</div>
			</div>
	...
	<script type="text/javascript">
		function removeBoard(num){
			if (confirm("정말 삭제하시겠습니까?")) {
				location.href="removeBoard/"+num;	
			}
		}
		function updateBoard(num){
			location.href="board_modify/"+num;	
		}
		...
		function removeComment(no){
			if (confirm("정말 삭제하시겠습니까?")) {
				location.href="removeComment/"+no;	
			}
		}
		/*댓글변경 버튼*/
		var num=${board.num}
