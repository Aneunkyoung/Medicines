    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-6 col-sm-offset-3">
                <h2 class="module-title align-center font-alt" onclick="location.href='${pageContext.request.contextPath}/admin_notice'" style="cursor: pointer;">Notice Page</h2>
        ...
        <form id="noticeForm" action="${pageContext.request.contextPath }/admin_noticeRemove" method="post">
            <table border="1" style="width: 100%; border: 1px solid #ddd; border-spacing: 10px 10px;">
                <tr style="border-bottom: 1px solid #ddd;">
                    ...
                    <c:forEach items="${noticeList}" var="noticeList">
                        <tr style="cursor: pointer;">
                            <td>
                                <input type="checkbox" name="checkNum" value="${noticeList.num}" class="check"></td>
                                <td onclick="location.href='${pageContext.request.contextPath}/notice_view?num=${noticeList.num}'">${noticeList.id }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/notice_view?num=${noticeList.num}'">${noticeList.writer }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/notice_view?num=${noticeList.num}'">${noticeList.title }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/notice_view?num=${noticeList.num}'">${noticeList.content }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/notice_view?num=${noticeList.num}'">${noticeList.regdate }</td>
                            </tr>
                        </c:forEach>
                    </table>
                </form>
                <div style="margin-top: 10px;">
                    <div style="float: left;">
                        <form method="post" action="${pageContext.request.contextPath}/admin_notice?pageNum=${pager.pageNum}">
                ...
                <div id="paging" class="pagination font-inc text-uppercase">
                    <c:choose>
                        <c:when test="${pager.startPage > pager.blockSize}">
                            <a href="${pageContext.request.contextPath}/admin_notice?pageNum=1&search_option=${search_option}&keyword=${keyword}">
                                <i class='fa fa-angle-left'></i>
                                First
                            </a>
                            <a href="${pageContext.request.contextPath}/admin_notice?pageNum=${pager.prevPage}&search_option=${search_option}&keyword=${keyword}">
                                <i class='fa fa-angle-left'></i>
                                Prev
                            </a>
                        </c:when>
                    ...
                    <c:forEach begin="${pager.startPage }" end="${pager.endPage }" var="page">
                        <c:choose>
                            <c:when test="${page != pager.pageNum }">
                                <a href="${pageContext.request.contextPath}/admin_notice?pageNum=${page }&search_option=${search_option}&keyword=${keyword}">[${page}]</a>
                            </c:when>
                            <c:otherwise>
                                [${page}]
                            </c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${pager.endPage != pager.totalPage}">
                            <a href="${pageContext.request.contextPath}/admin_notice?pageNum=${pager.nextPage}&search_option=${search_option}&keyword=${keyword}">
                                Next
                                <i class='fa fa-angle-right'></i>
                            </a>
                            <a href="${pageContext.request.contextPath}/admin_notice?pageNum=${pager.totalPage}&search_option=${search_option}&keyword=${keyword}">
                                Last
                                <i class='fa fa-angle-right'></i>
                            </a>
                        </c:when>
