...
<section class="module">
    <div class="container">
        ...
        <form id="questionForm" action="${pageContext.request.contextPath }/admin_questionRemove" method="post">
	        <table border="1" style="width: 100%; border: 1px solid #ddd; border-spacing: 10px 10px;">
	            ...
	            <c:forEach items="${questionList}" var="questionList">
	                <tr style="cursor: pointer;">
	                	<td><input type="checkbox" name="checkNum" value="${questionList.num}" class="check"></td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/question_view?num=${questionList.num}'">${questionList.id }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/question_view?num=${questionList.num}'">${questionList.writer }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/question_view?num=${questionList.num}'">${questionList.title }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/question_view?num=${questionList.num}'">${questionList.content }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/question_view?num=${questionList.num}'">${questionList.regdate }</td>
	                </tr>
	            </c:forEach>
	        </table>
	       </form>
        ...
        <div style="margin-top: 10px;">
            <div style="float: left;">
                <form method="post" action="${pageContext.request.contextPath}/admin_questionList?pageNum=${pager.pageNum}">
        ...
        <div id="paging" class="pagination font-inc text-uppercase">
            <c:choose>
                <c:when test="${pager.startPage > pager.blockSize}">
                    <a href="${pageContext.request.contextPath}/admin_questionList?pageNum=1&search_option=${search_option}&keyword=${keyword}">
                        <i class='fa fa-angle-left'></i>
                        First
                    </a>
                    <a href="${pageContext.request.contextPath}/admin_questionList?pageNum=${pager.prevPage}&search_option=${search_option}&keyword=${keyword}">
                        <i class='fa fa-angle-left'></i>
                        Prev
                    </a>
                </c:when>
            ...
            <c:forEach begin="${pager.startPage }" end="${pager.endPage }" var="page">
                <c:choose>
                    <c:when test="${page != pager.pageNum }">
                        <a href="${pageContext.request.contextPath}/admin_questionList?pageNum=${page }&search_option=${search_option}&keyword=${keyword}">[${page}]</a>
                    </c:when>
                    <c:otherwise>
                        [${page}]
                    </c:otherwise>
            ...
            <c:choose>
                <c:when test="${pager.endPage != pager.totalPage}">
                    <a href="${pageContext.request.contextPath}/admin_questionList?pageNum=${pager.nextPage}&search_option=${search_option}&keyword=${keyword}">
                        Next
                        <i class='fa fa-angle-right'></i>
                    </a>
                    <a href="${pageContext.request.contextPath}/admin_questionList?pageNum=${pager.totalPage}&search_option=${search_option}&keyword=${keyword}">
                        Last
                        <i class='fa fa-angle-right'></i>
                    </a>
                </c:when>
