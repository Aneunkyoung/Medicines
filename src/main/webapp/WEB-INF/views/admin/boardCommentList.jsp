...
<section class="module">
    <div class="container">
        ...
        <form id="boardCommentForm" action="${pageContext.request.contextPath }/admin_boardCommentRemove" method="post">
	        <table border="1" style="width: 100%; border: 1px solid #ddd; border-spacing: 10px 10px;">
	            ...
	            <c:forEach items="${boardCommentList}" var="boardCommentList">
	                <tr style="cursor: pointer;">
	                	<td><input type="checkbox" name="checkNo" value="${boardCommentList.no}" class="check"></td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardCommentList.boardNo}'" >${boardCommentList.id }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardCommentList.boardNo}'" >${boardCommentList.writer }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardCommentList.boardNo}'" >${boardCommentList.content }</td>
	                	<td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardCommentList.boardNo}'" >${boardCommentList.regdate }</td>
	                </tr>
	            </c:forEach>
	        </table>
        </form>
        ...
        <div style="margin-top: 10px;">
            <div style="float: left;">
                <form method="post" action="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=${pager.pageNum}">
        ...
        <div id="paging" class="pagination font-inc text-uppercase">
            <c:choose>
                <c:when test="${pager.startPage > pager.blockSize}">
                    <a href="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=1&search_option=${search_option}&keyword=${keyword}">
                        <i class='fa fa-angle-left'></i>
                        First
                    </a>
                    <a href="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=${pager.prevPage}&search_option=${search_option}&keyword=${keyword}">
                        <i class='fa fa-angle-left'></i>
                        Prev
                    </a>
                </c:when>
            ...
            <c:forEach begin="${pager.startPage }" end="${pager.endPage }" var="page">
                <c:choose>
                    <c:when test="${page != pager.pageNum }">
                        <a href="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=${page }&search_option=${search_option}&keyword=${keyword}">[${page}]</a>
                    </c:when>
                    <c:otherwise>
                        [${page}]
                    </c:otherwise>
            ...
            <c:choose>
                <c:when test="${pager.endPage != pager.totalPage}">
                    <a href="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=${pager.nextPage}&search_option=${search_option}&keyword=${keyword}">
                        Next
                        <i class='fa fa-angle-right'></i>
                    </a>
                    <a href="${pageContext.request.contextPath}/admin_boardCommentList?pageNum=${pager.totalPage}&search_option=${search_option}&keyword=${keyword}">
                        Last
                        <i class='fa fa-angle-right'></i>
                    </a>
                </c:when>
