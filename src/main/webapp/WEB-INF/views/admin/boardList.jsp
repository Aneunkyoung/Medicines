...
<section class="module">
    <div class="container">
        ...
        <form id="boardForm" action="${pageContext.request.contextPath }/admin_boardRemove" method="post">
            <table border="1" style="width: 100%; border: 1px solid #ddd; border-spacing: 10px 10px;">
                <tr style="border-bottom: 1px solid #ddd;">
                    ...
                    <c:forEach items="${boardList}" var="boardList">
                        <tr style="cursor: pointer;">
                            <td>
                                <input type="checkbox" name="checkNum" value="${boardList.num}" class="check"></td>
                                <td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardList.num}'">${boardList.id }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardList.num}'">${boardList.writer }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardList.num}'">${boardList.title }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardList.num}'">${boardList.content }</td>
                                <td onclick="location.href='${pageContext.request.contextPath}/board_view?num=${boardList.num}'">${boardList.regdate }</td>
                            </tr>
                        </c:forEach>
                    </table>
                </form>
                <div style="margin-top: 10px;">
                    <div style="float: left;">
                        <form method="post" action="${pageContext.request.contextPath}/admin_boardList?pageNum=${pager.pageNum}">
                ...
                <div id="paging" class="pagination font-inc text-uppercase">
                    <c:choose>
                        <c:when test="${pager.startPage > pager.blockSize}">
                            <a href="${pageContext.request.contextPath}/admin_boardList?pageNum=1&search_option=${search_option}&keyword=${keyword}">
                                <i class='fa fa-angle-left'></i>
                                First
                            </a>
                            <a href="${pageContext.request.contextPath}/admin_boardList?pageNum=${pager.prevPage}&search_option=${search_option}&keyword=${keyword}">
                                <i class='fa fa-angle-left'></i>
                                Prev
                            </a>
                        </c:when>
                    ...
                    <c:forEach begin="${pager.startPage }" end="${pager.endPage }" var="page">
                        <c:choose>
                            <c:when test="${page != pager.pageNum }">
                                <a href="${pageContext.request.contextPath}/admin_boardList?pageNum=${page }&search_option=${search_option}&keyword=${keyword}">[${page}]</a>
                            </c:when>
                            <c:otherwise>
                                [${page}]
                            </c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${pager.endPage != pager.totalPage}">
                            <a href="${pageContext.request.contextPath}/admin_boardList?pageNum=${pager.nextPage}&search_option=${search_option}&keyword=${keyword}">
                                Next
                                <i class='fa fa-angle-right'></i>
                            </a>
                            <a href="${pageContext.request.contextPath}/admin_boardList?pageNum=${pager.totalPage}&search_option=${search_option}&keyword=${keyword}">
                                Last
                                <i class='fa fa-angle-right'></i>
                            </a>
                        </c:when>
